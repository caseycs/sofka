...
| `enter`                                       | drill down (workload/svc → pods, node → its pods, pod → containers, ns → re-scope, CRD → its resources, NodeClaim → its node)         |
...
| `o`                                           | show the node hosting the selected pod, or backing the selected Karpenter NodeClaim                                                   |
